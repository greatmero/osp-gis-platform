...
import { Asset, AssetType } from '../../api/client';
...
type AssetWithRelations = Asset & {
  assetType?: AssetType;
  connectionsFrom?: Array<{ id: number; toAsset: { name: string }; fiberCount: number }>;
  connectionsTo?: Array<{ id: number; fromAsset: { name: string }; fiberCount: number }>;
  incidents?: Array<{ id: number; description: string; severity: string; status: string; openedAt: string }>;
};
          ...
          {asset && (
            <>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`text-xs px-2 py-0.5 rounded font-medium ${STATUS_STYLES[asset.status] ?? 'bg-gray-700 text-gray-300'}`}>
                  {STATUS_LABELS[asset.status] ?? asset.status}
                </span>
                {assetType && (
                  <span className="flex items-center gap-1.5 text-xs text-gray-400">
                    <span className="w-2 h-2 rounded-full" style={{ background: assetType.color }} />
                    {assetType.label}
                  </span>
                )}
              </div>
              ...
              {/* Connections */}
              {rich && (rich.connectionsFrom?.length || rich.connectionsTo?.length) ? (
                <div>
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                    Connections
                  </h3>
                  <div className="space-y-1">
                    {(rich.connectionsFrom ?? []).map((c) => (
              ...
              {/* Recent Incidents */}
              {rich?.incidents && rich.incidents.length > 0 && (
                <div>
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                    Recent Incidents
                  </h3>
                  <div className="space-y-2">
                    {rich.incidents.slice(0, 5).map((inc) => (
        ...
        {asset && assetType && import.meta.env.VITE_DEMO_MODE !== 'true' && (
          <div className="px-4 py-3 border-t border-gray-700 flex gap-2 shrink-0">
            <button
              ...
              className="flex-1 text-xs bg-blue-600 hover:bg-blue-700 text-white rounded-md py-2 font-medium transition-colors"
            >
              Edit
            </button>
            <button
              onClick={() => setDeleteConfirm(true)}
              className="flex-1 text-xs bg-gray-800 hover:bg-red-900/50 text-gray-300 hover:text-red-400 rounded-md py-2 font-medium transition-colors border border-gray-700 hover:border-red-800"
            >
              Delete
            </button>
          </div>
